                                    ...
                                    <div class="form-group row">
                                        @foreach ($amenitiesdata as $item)
                                            <div class="col-sm-4 mb-2 mb-sm-0">
                                                <input type="checkbox" id="smoking_allowed{{ $loop->index }}"
                                                    name="camera_video[]" value="{{ $item['id'] }}">
                                                <label for="smoking_allowed">{{ $item['name'] }}</label>
                                            </div>
                                        @endforeach
                                    </div>
...
@section('script')
    <script>
        ...
        function addCheck(check, id) {
            console.log(check);
            if (check) {
                if (checkedDate.indexOf(id) != -1) {
                    checkedDate.push(id);
                }
            } else {
                if (checkedDate.indexOf(id) != -1) {
                    checkedDate.splice(checkedDate.indexOf(id), 0);
                }
            }
                    ...
                    success: function(res) {
                        ...
                        if (res.status == 400) {
                            ...
                            if (res.length > 0) {
                                ...
                                window.location = '/addhostawaylocation'
                            } else {
                                alert('Please Select Alteast One Feature');
                            }
                        }
                    }
                });
